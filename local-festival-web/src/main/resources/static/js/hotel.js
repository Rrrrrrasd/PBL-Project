...
function addHotelToPanel(hotel) {
    ...
    const distanceText = hotel.dist ? `${distanceMeters}m` : '거리 정보 없음';
...
function loadMoreHotels(pageNo) {
    ...
    fetch(`/api/festivals/hotels/more?mapx=${window.currentMapX}&mapy=${window.currentMapY}&pageNo=${pageNo}&numOfRows=${numOfRows}`)
...
function updateLoadMoreHotelStatus(totalCount, buttonId) {
    ...
    if (totalCount <= window.currentPageHotels * numOfRows) {
