...
function addRestaurantToPanel(restaurant) {
    ...
    console.log(`Parsed distanceMeters: ${distanceMeters} for restaurant: ${restaurant.title || '제목 없음'}`);
    ...
    console.log(`Formatted distanceText: ${distanceText} for restaurant: ${restaurant.title || '제목 없음'}`);
    ...
    div.innerHTML = `
	        <div class="recommendationInform section">
	            ...
	            <div>
					...
					<button class="otherInfoBTN restaurant-marker-btn show-marker-btn marker-button" id="Directions">
...
document.getElementById('loadMoreRestaurantsButton').addEventListener('click', function () {
    if (window.totalRestaurants > window.currentPageRestaurants * window.numOfRows) {
        window.currentPageRestaurants += 1;
...
function loadMoreRestaurants(pageNo) {
    ...
    fetch(`/api/festivals/restaurants/more?mapx=${window.currentMapX}&mapy=${window.currentMapY}&pageNo=${pageNo}&numOfRows=${window.numOfRows}`)
...
function updateLoadMoreRestaurantStatus(totalCount, buttonId) {
    ...
    if (totalCount <= window.currentPageRestaurants * window.numOfRows) {
...
function addRestaurantMarker(restaurant) {
    console.log('Adding marker for restaurant id: ', restaurant.uniqueId);
    ...
    if (restaurantButton) {
        restaurantButton.addEventListener('click', function () {
            ...
            if (window.cache[key] && window.cache[key].restaurants.length > 0) {
                ...
                console.log(`Restaurants cache : ${JSON.stringify(window.cache[key].restaurants)}`)
            }
            else {
                loadMoreRestaurants(window.currentPageRestaurants);
